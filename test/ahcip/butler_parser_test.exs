defmodule AHCIP.ButlerParserTest do
  ...
  setup_all do
    if File.exists?(@butler_path) do
      {:ok, butler: AHCIP.ButlerParser.parse_file(@butler_path)}
    else
      {:ok, butler: nil}
    end
  end
  ...
  test "lookup returns text for a line range", %{butler: butler} do
    if butler do
      text = AHCIP.ButlerParser.lookup(butler, 1, 1, 5)
  ...
  test "lookup returns empty string for nonexistent book", %{butler: butler} do
    if butler do
      assert AHCIP.ButlerParser.lookup(butler, 99, 1, 10) == ""
    end
  end
